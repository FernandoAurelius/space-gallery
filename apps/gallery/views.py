...
def upload(request):
    ...
    if request.method == "POST":
        form = PhotographyForm(request.POST, request.FILES, user=request.user)
        if form.is_valid():
            form.save()
            messages.success(request, "Fotografia foi salva com sucesso!")
            return redirect("index")
    else:
        form = PhotographyForm(user=request.user)
...
def filt(request, category):
    photographys = Photography.objects.order_by("-release_date").filter(
